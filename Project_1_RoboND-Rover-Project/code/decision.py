...
def decision_step(Rover):
    ...
    if Rover.nav_angles is not None:
        ...
        if Rover.mode == 'forward': 
            ...
            if Rover.vel <= 2.5 and Rover.vel >= 2.4 and Rover.steer <= 15 and Rover.steer >= 14 and not Rover.is_in_loop:
                print("inloop")
                if Rover.start_loop_time is None:
                    Rover.start_loop_time = Rover.total_time
                if Rover.total_time - Rover.start_loop_time >= Rover.loop_turning_waiting_time:
                    print("correctloop")
                    Rover.is_in_loop = True
            elif Rover.is_in_loop and (Rover.throttle != 0 or Rover.brake !=0):
                if Rover.vel >= 2.3 or Rover.vel <= 2.5:
                    Rover.start_loop_time = None
                    Rover.is_in_loop = False
                    Rover.mode = 'stop'
                    Rover.steer = np.clip(mean_without_outlier(Rover.nav_angles * 180/np.pi), -5, 5)
                else:
                    Rover.brake = 0
                    Rover.throttle = -Rover.throttle_set
                    Rover.steer = 0
                    if ((Rover.total_time - Rover.start_loop_time) // Rover.loop_turning_waiting_time) % 2 == 0:
                        Rover.steer = np.clip(-(Rover.steer + 5), -15, 15)
                    if ((Rover.total_time - Rover.start_loop_time) // Rover.loop_turning_waiting_time) % 5 == 0:
                        Rover.throttle = 0
                    if ((Rover.total_time - Rover.start_loop_time) // Rover.loop_turning_waiting_time) % 6 == 0:
                        Rover.throttle = Rover.throttle_set
            else:
                Rover.is_in_loop = False
                Rover.start_loop_time = None
